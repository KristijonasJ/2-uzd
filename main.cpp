...
int main()
{
    ...
    {
        Gen(rekursija, n, k, failas0, namudarbuskc, mt);
    ...
    while(1)
    {
        ...
        {
            a++;
            pazymys.pazymys.push_back(rand()%10+1) ;
            cout << pazymys.pazymys[j] << endl;
            kiekis+=pazymys.pazymys[j];
        }
    }
    ...
    if(MV=='V')
    pazymys.galutinis = 0.4*(kiekis/a)+ (0.6*egzaminas);
    else if(MV=='M')
    {
         sort(pazymys.pazymys.begin(),pazymys.pazymys.end());
        if(a%2!=0)
    ...
    pazymys.galutinis = (0.4*mediana) +(0.6*egzaminas);
    }
   ...
   if(MV=='M')
   {
       ...
       for(list<pazymiai>::iterator i=M.begin(); i!=M.end(); i++)
       {
           ...
           fr << i->vardas;
           fr << setw(20);
           fr << fixed<<setprecision(2) << i->galutinis << endl;
       }
   }
   else if (MV=='V')
   {
